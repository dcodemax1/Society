...
function ReferralPage() {
  ...
  useEffect(() => {
    const loadReferrals = async () => {
      try {
        setLoading(true);
        const response = await memberApi.getReferrals();
        if (response?.success) {
  ...
  const stats = {
    ...
    topReferrer:
      allReferrals.length > 0
        ? allReferrals.reduce((max, curr) =>
            curr.totalReferrals > max.totalReferrals ? curr : max
          )
        : { referrerName: "N/A", totalReferrals: 0 },
  };
            ...
            {loading && (
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  <p className="text-gray-600 mb-4">Loading referral data...</p>
