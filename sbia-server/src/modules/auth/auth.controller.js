...
export const login = async (req, res) => {
  try {
    ...
    return res.json(success("Login successful", { accessToken, user: { id: user.id, email: user.email, is_verified: user.is_verified } }));
  } catch (err) {
...
export const logout = async (req, res) => {
  try {
    ...
    if (rt) {
      ...
      const payload = (() => {
        try { return jwt.verify(rt, process.env.JWT_REFRESH_SECRET); } catch { return null; }
      })();
...
export const me = async (req, res) => {
  ...
  // minimal user object
  return res.json(success("me", { id: req.user.id, email: req.user.email, is_verified: req.user.is_verified }));
};